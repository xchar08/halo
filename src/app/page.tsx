...
async function createProject(formData: FormData) {
  ...
  if (!user) {
      // Should be caught by the page redirect, but double-check here
      redirect("/login");
  }
...
export default async function Home() {
  ...
  // AUTH CHECK: Protect the dashboard
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) {
      redirect("/login");
  }
  ...
  // Note: If RLS is enabled, this filter happens automatically on the DB side.
  // If RLS is disabled, we explicitly filter by owner_id for safety in MVP.
    ...
    <main className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-[#050a14] to-black text-zinc-50 p-8 md:p-24 relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto space-y-12 relative z-10">
        ...
        <RevealWrapper>
            ...
            {/* Active "Create" Card - Glassy & Glowing */}
                ...
                <Card className="bg-zinc-900/40 backdrop-blur-md border-white/10 relative overflow-visible shadow-2xl">
                    ...
                    <CardContent>
                        ...
                        <div className="space-y-2 flex-1 w-full">
                            ...
                            <Input 
                                name="name" 
                                placeholder="e.g. Orbital Mechanics Analysis" 
                                className="bg-black/30 border-white/10 text-white placeholder:text-white/20 focus:ring-cyan-500/50 focus:border-cyan-500/50 transition-all"
                                required
                            />
                        </div>
                        <div className="space-y-2 flex-[2] w-full">
                            ...
                            <Input 
                                name="query" 
                                placeholder="What mystery shall we solve?" 
                                className="bg-black/30 border-white/10 text-white placeholder:text-white/20 focus:ring-yellow-500/50 focus:border-yellow-500/50 transition-all"
                            />
                        </div>
        ...
        <RevealWrapper delay={0.2}>
            ...
            {projects?.map((project) => (
                ...
                <Card className="h-full bg-zinc-900/30 backdrop-blur-sm border-white/5 hover:bg-zinc-900/50 transition-all hover:border-cyan-500/30 cursor-pointer group relative overflow-hidden">
                    ...
                    <CardHeader>
                    <CardTitle className="text-lg text-zinc-200 group-hover:text-cyan-300 transition-colors">
                        {project.name}
                    </CardTitle>
                    </CardHeader>
                    <CardContent>
                    <p className="text-sm text-zinc-400 group-hover:text-zinc-300 transition-colors line-clamp-3">
            ...
            {(!projects || projects.length === 0) && (
                <div className="col-span-full text-center py-12 text-zinc-600/50 border border-dashed border-white/5 rounded-lg bg-black/20">
                The void is empty. Illuminate it with a new project.
                </div>
            )}
            </div>
        </RevealWrapper>
