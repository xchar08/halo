...
export default function ProjectPage() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
          console.log("Initializing Project Page (RxDB Disabled)...");
          ...
          // NOTE: RxDB initialization is DISABLED to avoid DB9 errors
          // Data will be fetched directly from Supabase via Realtime
    ...
    // Realtime Listener (replaces RxDB sync)
    const channel = supabase.channel(`project-${projectId}`)
        .on('postgres_changes', { 
            ...
            // Re-fetch graph on new documents
            getGraphSnapshot(projectId).then(s => {
                if (mounted) setGraphData(s.nodes, s.edges);
            });
        })
        .subscribe();
      ...
      <div className="flex-1 relative h-full flex flex-col">
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-50 pointer-events-auto">
            <div className="bg-zinc-900/90 backdrop-blur border border-zinc-800 p-1 rounded-lg flex gap-1 shadow-xl">
                <button onClick={() => setActiveTab("graph")} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${activeTab === "graph" ? "bg-zinc-800 text-white shadow-sm" : "text-zinc-400 hover:text-zinc-200"}`}>
                    Graph View
                </button>
                <button onClick={() => setActiveTab("report")} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${activeTab === "report" ? "bg-zinc-800 text-white shadow-sm" : "text-zinc-400 hover:text-zinc-200"}`}>
                    Final Report
                </button>
            </div>
        </div>
      ...
      <div className="w-[400px] border-l border-zinc-800 bg-zinc-950/95 backdrop-blur z-30 flex flex-col h-full shadow-xl">
        <div className="p-2 border-b border-zinc-800 flex gap-1 bg-zinc-900/50">
            <button 
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2 text-xs font-medium rounded-md transition-colors ${sidebarTab === 'agent' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
            >
                <Activity className="h-3.5 w-3.5" /> Agent Logs
            </button>
            <button 
                onClick={() => setSidebarTab("global")} 
                className={`flex-1 flex items-center justify-center gap-2 py-2 text-xs font-medium rounded-md transition-colors ${sidebarTab === 'global' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
            >
                <Globe className="h-3.5 w-3.5" /> Global Feed
            </button>
        </div>
